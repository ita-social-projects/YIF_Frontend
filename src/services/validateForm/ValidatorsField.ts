...
export const validationField = Yup.object().shape({
  ...
  universitySite: Yup.string()
    .matches(
      /^(http\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/,
      'Адресу сайту введено не вірно, адреса повинна починатись з http:// або https://'
    )
    .min(2, 'Замала кількість введених символів')
    .max(50, 'Кількість введених символів перевищує допустиме значення')
    .required('Заповніть поле'),
  ...
  lat: Yup.string()
    .min(2, 'Номер телефону має містити не менше ніж 12 символів')
    .required('Вкажіть розташування'),
