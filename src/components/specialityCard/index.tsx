...
interface Props {
  ...
  let { id, abbreviation, subjects, description } = props;
  const handlerClick = () => setOpened(!opened);  
  const result = subjects.map((item: any, idx: number) => (
    <li key={idx} className={styles.card__content__line}>
        <div className={styles.card__content_subject}>{item.name}</div>
        <div className={styles.card__content_mark}>мінімум <strong>{item.mark}</strong> балів</div>
    ...
    >
      <div className={`container`}>
             <div className={styles.card}>
                ...
                <h2 className={styles.subtitle}>{abbreviation} </h2>
                ...
                <div className = {`${styles.card__content__star}`}>
                  {starSVG}
                  </div>
                </div>
                  ...
                  <div className={`${styles.card__more} ${opened ? `${styles.card__more__opened}` : ""}`}>
                  <p className={styles.card__details}>
                    {description}
                    </p>
                    <div data-testid='open' className={styles.card__more__btn}   onClick={handlerClick}>
