...
const SpecialtiesList = (props: any) => {
          ...
          bodyContent={
            details.map((item: any) => (
              <li key={item.code} className={styles.acc_item__subitem}>
                ...
                <div className={`${styles.acc_item__block} ${styles.read__more} ${opened ? `${styles.read__more__opened}` : ''}`}>
                  <p><strong>Опис:</strong>  <span className={`${styles.acc_item__underline}`} onClick={handleClick}  data-testid='check-open'> натисніть, щоб показати </span></p>
  ...
  return (
    <section className={styles.specialityAdminPage}>
      <div className={styles.specialityAdminPage__top}>
        ...
        <button onClick={props.onChangeBlock} className={` ${styles.animatedButton}`} id='moreDetailsButton' data-testid='add-specialty'>Додати спеціальність</button>
      </div>
